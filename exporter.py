...
class LocustCollector(object):
    ...
    def collect(self):
        ...
        metric.add_sample('locust_user_count',
                          value=response['user_count'], labels={})
        yield metric

        metric = Metric('locust_errors', 'Locust requests errors', 'gauge')
        for err in response['errors']:
            metric.add_sample('locust_errors', value=err['occurences'], labels={
                              'path': err['name'], 'method': err['method']})
        yield metric
        ...
        if 'slave_count' in response:
            ...
            metric.add_sample('locust_slave_count',
                              value=response['slave_count'], labels={})
            yield metric

        metric = Metric('locust_fail_ratio', 'Locust failure ratio', 'gauge')
        metric.add_sample('locust_fail_ratio',
                          value=response['fail_ratio'], labels={})
        yield metric

        metric = Metric('locust_state', 'State of the locust swarm', 'gauge')
        metric.add_sample('locust_state', value=1, labels={
        ...
        for mtr in stats_metrics:
            ...
            if mtr in ['num_requests', 'num_failures']:
                mtype = 'counter'
            metric = Metric('locust_requests_' + mtr,
                            'Locust requests ' + mtr, mtype)
            for stat in response['stats']:
                if not 'Total' in stat['name']:
                    metric.add_sample('locust_requests_' + mtr, value=stat[mtr], labels={
